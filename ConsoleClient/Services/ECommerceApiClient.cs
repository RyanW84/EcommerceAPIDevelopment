...
public class ECommerceApiClient
{
    ...
    public async Task<ApiResult<bool>> DeleteProductAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        return ApiResult<bool>.Failure(error.Message, response.StatusCode);
    }
    ...
    private static async Task<ApiResult<T>> ReadResultAsync<T>(HttpResponseMessage response, CancellationToken cancellationToken)
    {
        if (response.IsSuccessStatusCode)
        {
            ...
            var data = await response.Content.ReadFromJsonAsync<T>(SerializerOptions, cancellationToken);
            return ApiResult<T>.Success(data, response.StatusCode);
        }

        var error = await ReadErrorAsync(response, cancellationToken);
        return ApiResult<T>.Failure(error.Message, response.StatusCode);
    }
        ...
        CancellationToken cancellationToken)
    {
        if (response.IsSuccessStatusCode)
        {
            ...
            return ApiResult<PaginatedResponse<T>>.Success(body, response.StatusCode);
        }

        var error = await ReadErrorAsync(response, cancellationToken);
        return ApiResult<PaginatedResponse<T>>.Failure(error.Message, response.StatusCode);
    }
...
public record ApiResult<T>(bool Success, T? Data, string Message, HttpStatusCode StatusCode)
{
    public static ApiResult<T> Success(T? data, HttpStatusCode statusCode) => new(true, data, string.Empty, statusCode);
    public static ApiResult<T> Failure(string message, HttpStatusCode statusCode) => new(false, default, message, statusCode);
}
